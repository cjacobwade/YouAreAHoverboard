...
public static class WadeUtils
{
	...
	public static HSVColor RGBToHSV( Color color )
	{
		...
		if ( max > min )
		{
			if ( WadeUtils.IsEqual( g, max ) )
			{
				ret.h = (v - r) / dif * 60f + 120f;
			}
			else if ( WadeUtils.IsEqual( v, max ) )
			{
...
public struct HSVColor
{
	...
	public static HSVColor Lerp( HSVColor a, HSVColor b, float t )
	{
		...
		if( WadeUtils.IsEqual( a.v, 0f ) )
		{
			h = b.h;
			s = b.s;
		}
		else if( WadeUtils.IsEqual( b.v, 0f ))
		{
			h = a.h;
			s = a.s;
		}
		...
		{
			if( WadeUtils.IsEqual( a.s, 0f ) )
			{
				h = b.h;
			}
			else if( WadeUtils.IsEqual( b.s, 0f ) )
			{
				h = a.h;
			}
			else
